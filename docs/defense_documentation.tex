...
\begin{tikzpicture}[
    ...
    \node[attacker] (atk) at (0,0) {\textbf{Attacker}\\192.168.56.20};
    \node[iptables] (fw) at (4,0) {iptables\\REDIRECT\\rule};
    \node[server] (srv) at (8,1) {\textbf{Real Server}\\192.168.56.10};
...
\begin{itemize}[leftmargin=*, itemsep=3pt]
    \item \textbf{Ubuntu Server} (192.168.56.10): AEGIS defense + ML models + Dashboard
    \item \textbf{Kali Linux} (192.168.56.20): Attacker machine running Nmap, custom scanners, and attack scripts
    \item \textbf{Network}: VMware NAT, 192.168.56.0/24
