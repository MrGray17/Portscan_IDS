...
% ============================================
% Adapted from ml_pipeline_report.tex by Khadija + Adil
...
In the context of the \textbf{AEGIS Entropy} project, the artificial intelligence-based detection module constitutes the core component of the defense system against \textit{Port Scan} attacks. This chapter details every step of the data pipeline: from exploring and cleaning the raw data, through selecting features according to the Data Dictionary, to preparing the final dataset for model training, strictly adhering to the PDCA (\textit{Plan-Do-Check-Act}) methodology and the academic standards taught in class.
...
\section{Feature Selection (Data Dictionary)}
...
\subsection{Outlier Detection and Handling (IQR Method)}
...
\subsection{Skewness Analysis and Logarithmic Transformation}
...
This chapter covered the complete data exploration and preparation pipeline applied to the CICIDS2017 PortScan dataset. Through descriptive analysis, we identified the key statistical properties and quality issues of the raw data. A series of preprocessing steps --- cleaning, IQR outlier detection, skewness correction, standardization, and SMOTE class balancing --- were then applied to produce a clean, model-ready dataset of 11 features. The next chapter will present the model training and evaluation phase.
